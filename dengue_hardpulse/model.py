...
def _bounded_log_rate_per_100k(
    name: str,
    center: float = 200.0,
    low: float = 1.0,
    high: float = 3_000.0,
    ...
    """Sample a bounded annual incidence baseline and return log per-person rate."""
    raw = numpyro.sample(
        f"{name}_raw",
        dist.Normal(_logit((center - low) / (high - low)), scale),
    )
    rate_100k = low + (high - low) * jax.nn.sigmoid(raw)
    numpyro.deterministic(f"{name}_per_100k", rate_100k)
    return jnp.log(rate_100k / 100_000.0)
...
def _standardize_city_vector(x: jnp.ndarray) -> jnp.ndarray:
    """Center and scale a city-level covariate inside the current fit."""
    x = jnp.asarray(x, dtype=jnp.float32)
    return (x - x.mean()) / jnp.maximum(x.std(), 1e-6)


def _sample_log_rate_base(data: dict) -> jnp.ndarray:
    """Absolute state incidence baseline plus tightly pooled city deviations.

    Population remains an exposure offset later in the mean equation.  This
    baseline is therefore a per-person dengue incidence risk, not raw cases.
    The state term carries the absolute incidence scale; city, population, and
    density terms explain systematic within-state differences without allowing
    each municipality to freely absorb outbreak noise.
    """
    C = int(data["n_cities"])
    Y = int(data["n_years"])
    ...
    with numpyro.plate("state_log_rate_base", n_state):
        log_rate_base_state = _bounded_log_rate_per_100k(
            "log_rate_base_state", center=200.0, high=3_000.0, scale=0.75
        )
    numpyro.deterministic("log_rate_base_state", log_rate_base_state)

    # Allow real within-state hotspot differences. The persistent state bias is
    # bounded separately, so this baseline can retain enough capacity to avoid
    # systematic near-zero forecasts.
    ...
    beta_log_pop = numpyro.sample("beta_log_pop_base", dist.Normal(0.0, 0.20))
    beta_log_density = numpyro.sample("beta_log_density_base", dist.Normal(0.0, 0.20))

    value = (
        log_rate_base_state[state_idx]
        + u_city
        + beta_log_pop * log_pop_z
        + beta_log_density * log_density_z
    )
    numpyro.deterministic("log_rate_base_city", value)
    numpyro.deterministic("base_rate_state_per_100k", 100_000.0 * jnp.exp(log_rate_base_state))
    numpyro.deterministic("base_rate_city_per_100k", 100_000.0 * jnp.exp(value))
    return value


def _annual_burden_features(data: dict, obs) -> tuple[jnp.ndarray, jnp.ndarray, jnp.ndarray]:
    """Return city immunity, city momentum, and state outbreak momentum.

    The city features track the unit's own observed annual incidence.  The
    state momentum aggregates cases and population before computing attack
    rate, giving the city-level forecast a smoother state outbreak-intensity
    signal. During forecasting, optional history_attack/state_history_attack
    carry training-period burden into the target grid without exposing target
    observations.
    """
    C = int(data["n_cities"])
    Y = int(data["n_years"])
    ...
    state_attack = jnp.log1p(10_000.0 * state_cases / state_pop)
...
def _sample_year_effect(Y: int, data: dict) -> jnp.ndarray:
    rho_year = _bounded_unit("rho_year_raw", center=0.35, scale=0.55, low=-0.10, high=0.80)
    sigma_year = numpyro.sample("sigma_year", dist.HalfNormal(0.10))
...
def _expected_cases(data: dict, obs, pulse: dict) -> jnp.ndarray:
    ...
    if state_profile is not None:
        ...
        with numpyro.plate("state_profile_weight_plate", n_state):
            state_profile_weight_raw = numpyro.sample(
                "state_profile_weight_raw",
                dist.Normal(_logit((0.90 - 0.60) / (0.99 - 0.60)), 0.45),
            )
        state_weight_state = 0.60 + 0.39 * jax.nn.sigmoid(state_profile_weight_raw)
    ...
    xi = _bounded_unit("xi_raw", center=0.45, scale=0.65, low=0.10, high=0.80)

    # Persistent state calibration absorbs systematic state-level bias that is
    # stable across validation splits, without forcing year/momentum terms to
    # compensate globally. Mean-centering keeps the national level identified.
    tau_state_amp_bias = _bounded_unit(
        "tau_state_amp_bias_raw", center=0.20, scale=0.50, low=0.02, high=0.50
    )
    ...
    # Recent burden is allowed to add only weak short-term persistence.
    # Larger past burden should mostly act through susceptible depletion below.
    beta_momentum = _bounded_unit(
        "beta_momentum_raw", center=0.03, scale=0.55, low=-0.05, high=0.12
    )
    beta_state_momentum = _bounded_unit(
        "beta_state_momentum_raw", center=0.05, scale=0.55, low=-0.05, high=0.15
    )
    ...
    eps_amp = sigma_amp * eps_amp_raw.T  # [C, Y]
    # Mean-center per city so log_rate_base captures the city-level mean rate.
    # Without this, eps_amp absorbs the city mean during training but reverts to
    # zero during forecasting (when eps_amp_raw is drawn from the prior), causing
    # systematic underprediction because the city-specific offset is lost.
    ...
    log_rate = (
        ...
        + year_effect[None, :]
        + state_amp_bias[data["state_idx"]][:, None]
        + eps_amp
        + beta_momentum * momentum
        + beta_state_momentum * state_momentum
        - xi * immunity
    )
